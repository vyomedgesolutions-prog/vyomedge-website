...
function Skeleton() {
  return (
    <div className="max-w-3xl mx-auto animate-pulse space-y-6 pt-32 px-6">
      ...
      {[...Array(6)].map((_, i) => (
        <div key={i} className="h-4 bg-t-skeleton rounded"
  ...
  return (
    <div className="min-h-screen pb-20">
      <SEO
        ...
        keywords={blog.seoKeywords || blog.tags?.join(", ") || "digital marketing, SEO, VyomEdge blog"}
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className="flex flex-wrap items-center gap-3 mb-5">
              ...
              <span className="text-t-muted text-xs">· By {blog.author || "VyomEdge"}</span>
              {blog.readTime > 0 && (
                <span className="text-t-muted text-xs">· {blog.readTime} min read</span>
              )}
            </div>
      ...
      {blog.featuredImage && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.2 }} className="max-w-4xl mx-auto px-6 mb-12">
      ...
      {/* Content */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.3 }} className="max-w-3xl mx-auto px-6">
            ...
            <div className="space-y-4">
              {blog.faqs.map((f, i) => (
                <details key={i} className="glass rounded-2xl p-6 group">
                  <summary className="text-t-text font-bold cursor-pointer list-none flex items-center justify-between">
                    {f.question}
                    <span className="text-[#4CFFE7] text-lg group-open:rotate-45 transition-transform duration-200">+</span>
                  </summary>
                  <p className="text-t-secondary text-sm leading-relaxed mt-3">
                    {f.answer}
                  </p>
                </details>
              ))}
